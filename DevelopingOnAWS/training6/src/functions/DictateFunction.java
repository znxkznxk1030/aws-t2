...
public class DictateFunction implements RequestHandler<DictateRequest, String> {
   ...
   * 1. Takes a JSON payload from API gateway and converts it into a
   * DictateRequest {@link com.amazonaws.pollynotes.pojo.DictateRequest}
   ...
   * 
   * @param request POJO of DictateRequest created from the JSON sent from API GW
   *                into Lambda
  ...
  public String handleRequest(DictateRequest request, Context context) {
    // The note object contains the voiceId, userId and noteId from the
    // /notes/{id}/POST
    // {
    // "voiceId": "...",
    // "note": {
    ...
    // Get the MP3 bucket name from env variable MP3_BUCKET_NAME in Lambda or from
    // the MP3_BUCKET_NAME variable
    ...
    // Save the audio stream returned by Amazon Polly to Lambda's /tmp directory to
    // be uploaded
    ...
    // Create the ObjectKey from the UserId and NoteId for uniqueness in the MP3
    // bucket
