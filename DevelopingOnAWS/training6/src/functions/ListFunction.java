...
public class ListFunction implements RequestHandler<Note, List<Note>> {
   ...
   * @return A List of Note containing all of the elements for the userId in
   *         DynamoDB
   */
  @Override
  public List<Note> handleRequest(Note note, Context context) {
    // The note object only contains the userId as it's the only parameter sent in
    // the /notes/GET
    // {
    // "userId": "..."
    // }
    ...
    // Create the Query Expression with the Hash/Partition Key from the note in the
    // request
