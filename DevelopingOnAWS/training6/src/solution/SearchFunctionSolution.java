...
public class SearchFunctionSolution implements RequestHandler<Note, List<Note>> {
   ...
   * 1. Takes a JSON payload from API gateway and converts it into a Note POJO
   * {@link com.amazonaws.pollynotes.pojo.Note}
   * 2. Queries DynamoDB using the QueryAsync feature. It will use the userId for
   * the Partition Key, but a Filter will need
   * to be applied to do the search based on the note text.
   * 3. Returns the List of Note found
   * 
   * @param note    POJO of Note created from the JSON sent from API GW into
   *                Lambda
   * @param context Lambda context which can be used for logging
   * @return A List of Note containing all of the items for the userId containing
   *         the note text in DynamoDB
   */
  @Override
  public List<Note> handleRequest(Note note, Context context) {
    // The note object contains the userId and a partial note sent from API GW in
    // the /notes/search/GET
    // {
    // "userId": "...",
    // "note": "..."
    // }
    ...
    // Create the Expression Attribute Value HashMap for the value of the note text
    // to search
    ...
    // Create the Query Expression with the Hash/Partition Key set to the one set in
    // the note in the request
    // and with the filter expression set to the value of the note sent in the
    // request
